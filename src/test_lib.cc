...
template <typename T, int T_WIDTH>
T ** allocInitBool2dArray(int rows, int cols) {
  // Allocate
  T **array = static_cast<T **>(malloc(sizeof(T *) * rows));
  for (int i = 0; i < rows; i++) {
    array[i] = static_cast<T *>(malloc(sizeof(T) * cols));
...
VTAGenericInsn getBGEMMInsn(int uop_size, bool reset) {
  // Converter
  union VTAInsn converter;
  // GEVM instruction initialization
  VTAGemInsn insn;
  insn.opcode = VTA_OPCODE_BGEMM;
  insn.reset_reg = reset;
  insn.uop_bgn = 0;
  insn.uop_end = uop_size;
  converter.gemm = insn;
          ...
          for (int dy = 0 - ypad; dy < ypad + 1; dy++) {
            for (int dx = 0 - xpad; dx < xpad + 1; dx++) {
              for (int ic = 0; ic < in_channels; ic++) {
                ...
                converter.gemm.dst_idx = ((b * height + y) * width + x) * out_channels + oc;
                converter.gemm.src_idx = ((b * height + y+dy) * width + x+dx)* in_channels + ic;
                converter.gemm.wgt_idx = ((oc * kheight + ypad+dy) * (kwidth) + xpad+dx)* in_channels + ic;
                uop_buf[uop_idx++] = converter.generic;
              }
            }
          }
...
VTAGenericUop * getBGEMMUops(int batch, int in_feat, int out_feat) {
  // Converter
  union VTAUop converter;
  // Derive the total uop size
...
int boolean_test(int batch, int in_channels, int out_channels) {
  // Some assertions
  assert(in_channels % VTA_BLOCK_IN == 0);
  assert(out_channels % VTA_BLOCK_OUT == 0);
  assert(batch % VTA_BATCH == 0);
  ...
  printf("=====================================================================================\n");
  printf("INFO - Boolean operation test: batch=%d, in_channels=%d, out_channels=%d\n",
         batch, in_channels, out_channels);
...
//#endif
  ...
  insn_buf[3] = getLoadStoreInsn(VTA_OPCODE_LOAD, VTA_MEM_ID_WGT, wgt_size);
  // Perform GEMM
  insn_buf[4] = getBGEMMInsn(uop_size, false);
  // Store output block
  insn_buf[5] = getLoadStoreInsn(VTA_OPCODE_STORE, VTA_MEM_ID_ACC, out_size);

  // Prepare the uop buffer
  VTAGenericUop * uop_buf = getBGEMMUops(batch / VTA_BATCH,
                                    in_channels / VTA_BLOCK_IN,
                                    out_channels / VTA_BLOCK_OUT);
  ...
  // Initialize inputs
  inp_T **inputs = allocInitBool2dArray<inp_T, VTA_INP_WIDTH>(batch, in_channels);
  // Initialize weights
  wgt_T **weights = allocInitBool2dArray<wgt_T, VTA_WGT_WIDTH>(out_channels, in_channels);
  // Initialize biases
  acc_T **biases = allocInit2dArray<acc_T, VTA_ACC_WIDTH>(batch, out_channels);
  ...
  if (err == 0) {
    printf("INFO - Boolean operation test successful!\n");
    return 0;
  } else {
    printf("INFO - Boolean operation test failed, got %d errors!\n", err);
    return -1;
  }
}
